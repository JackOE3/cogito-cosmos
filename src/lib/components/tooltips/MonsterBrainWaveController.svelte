<script lang="ts">
    ...
    import { type BrainMode, cheeseMonsterDeathRateStats, cheeseMonsterBrainModeResourceFactors, currentNotation } from '$lib/store'
...
<div class="tooltip-arrow-up" id="brainWaveInfo" {style}>
    <span style="text-decoration: underline; font-weight: bold; color: rgb(255, 0, 98); margin-bottom: 0.25rem"> Monster Info </span>
    <br />
    <span style="display:flex;flex-direction:column;gap:0.25rem;">
        <span>{brainModeDescription[data]}</span>
        <span class="effect">
            Death toll: {cheeseMonsterDeathRateStats[data] > 0 ? `${formatNumber(cheeseMonsterDeathRateStats[data], 2, $currentNotation)}/s/monster` : 'None'}
        </span>
        <span class="effect">Relative resource generation: {cheeseMonsterBrainModeResourceFactors[data]}x</span>
        <div style="opacity:var(--medium-emphasis); font-style: oblique; text-align: right; padding-left: 20px; margin-top: 0.25rem ">
