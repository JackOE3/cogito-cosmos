  ...
  <div style="display:grid; grid-template-columns: 226px auto; gap: 8px">
    <div>
      <span class="resourceDisplay"
        >You have {formatNumber($resource.milk, 2, $currentNotation)} <strong style="color:white">milk</strong> <br />
      </span>
    ...
    <div class="gridColumn">
      {#each milkUpgrades as upgrade}
        <div class="upgrade-button-container">
          ...
          >
            <img
              alt="extra info for {upgrade.title}"
              src="assets/chevron-arrow-down.png"
              style="width: 100%; transform: rotate(-90deg); filter: invert(100%);"
            />
          </button>
        </div>
      {/each}
